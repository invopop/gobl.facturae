...
func TestInvoiceLineIncludingTaxes(t *testing.T) {
	...
	assert.Equal(t, "3305.7851", line1.TotalCost)
	...
	assert.Equal(t, "3140.4959", line1.GrossAmount)

	tax1 := line1.TaxesOutputs.List[0]
	tax2 := line1.TaxesWithheld.List[0]
	assert.Equal(t, "01", tax1.Code)
	assert.Equal(t, "3140.4959", tax1.Base.TotalAmount)
	assert.Equal(t, "21.0", tax1.Rate)
	assert.Equal(t, "659.5041", tax1.Amount.TotalAmount)
	assert.Equal(t, "04", tax2.Code)
	assert.Equal(t, "3140.4959", tax2.Base.TotalAmount)
	assert.Equal(t, "15.0", tax2.Rate)
	assert.Equal(t, "471.0743", tax2.Amount.TotalAmount)
	...
	assert.Equal(t, "166.9421", line2.TotalCost)
	assert.Equal(t, "166.9421", line2.GrossAmount)
	assert.Equal(t, "01", tax1.Code)
	assert.Equal(t, "166.9421", tax1.Base.TotalAmount)
	assert.Equal(t, "21.0", tax1.Rate)
	assert.Equal(t, "35.0578", tax1.Amount.TotalAmount)
